...
function shoeSize( player_name ) {
    ...
    if ( theGame.home.players[player_name] ) {
        return theGame.home.players[player_name].shoe
    } else if ( theGame.away.players[player_name] ){
        return theGame.away.players[player_name].shoe
    } else {
        return undefined
    }
}
...
console.log('color',teamColors("Charlotte Hornets"))
...
function teamNames() {
   ...
   const teamNewNames=[];
    for( team in theGame ){
        ...
        teamNewNames.push(theGame[team].teamName)

    }
    return teamNewNames
    
        //  return [theGame.home.teamName, theGame.away.teamName];  
}
...
*/
 function playerNumber(team_name) {
    const theGame = gameObject()
    const newNumbers=[]
    for ( team in theGame ) {
        const wholeTeam = theGame[team].players
        for(player in wholeTeam){
            if ( theGame[team].teamName === team_name ) {
                newNumbers.push(wholeTeam[player].number)
             }
